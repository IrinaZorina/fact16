<?
/* Задание 6. Создайте анкету на форме. Ответ на каждый из вопросов представьте с помощью переключателя («да» или «нет»). 
   По умолчанию задайте везде значение «да».
   Напишите сценарий обработки анкеты: поставьте 1 балл, если был ответ «да» в вопросах с номерами 3, 9, 10, 13, 14, 19 
   и «нет» в вопросах с номерами 1, 2, 4, 5, 6, 7, 8, 11, 12, 15, 16, 17, 18. Подсчитайте сумму набранных баллов:
...
if (isset($_POST["submit"]) && !in_array($_POST['customer-name'], ["", null])) {
  ...
  if ($questionsKeys) {
    ...
    $message = $score > 15 ? "У Вас покладистый характер" : ($score >= 8 ?
      "Вы не лишены недостатков, но с вами можно ладить" : "Вашим друзьям можно посочувствовать");
